...
import { View, Text, StyleSheet, Dimensions, FlatList, useColorScheme, Platform, Image } from 'react-native';
import { router, Redirect } from 'expo-router';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import Animated, { FadeInDown, FadeIn, SlideInRight, LinearTransition } from 'react-native-reanimated';
...
export default function OnboardingScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? Semantic.background.dark : Semantic.background.primary }]}>
      <FlatList
        ref={flatListRef}
        data={SLIDES}
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom + Spacing.lg, Spacing.xxl) }]}>
        <View style={styles.pagination}>
          {SLIDES.map((_, i) => (
            <Animated.View 
              ...
              style={[
                styles.dot, 
                { backgroundColor: currentIndex === i ? Semantic.primary.main : (isDark ? Colors.obsidian[700] : Colors.neutral[200]) },
        ...
        <View style={styles.actions}>
          ...
          >
            <Text style={[styles.skipText, { color: getMutedColor() }]}>Lewati</Text>
          </AnimatedPress>
          ...
          <AnimatedPress 
            ...
            onPress={nextSlide}
          >
            <Text style={styles.nextText}>{currentIndex === SLIDES.length - 1 ? "Mulai" : "Lanjut"}</Text>
...
const styles = StyleSheet.create({
  ...
  iconContainer: { width: 200, height: 200, borderRadius: 100, alignItems: 'center', justifyContent: 'center', marginBottom: Spacing.xxxl },
  title: { fontFamily: Typography.fontFamily.primary, fontSize: 32, textAlign: 'center', lineHeight: 40, marginBottom: Spacing.lg },
  desc: { fontFamily: Typography.fontFamily.medium, fontSize: 16, textAlign: 'center', lineHeight: 24 },
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: Spacing.xl },
  pagination: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginBottom: Spacing.xxl },
  ...
  nextBtn: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 24, borderRadius: BorderRadius.full, ...Shadows.sm },
  nextText: { fontFamily: Typography.fontFamily.primary, fontSize: 16, color: '#FFF' }
});
